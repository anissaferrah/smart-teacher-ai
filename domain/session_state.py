...
"""
FIXED domain/session_state.py
FIX-1: CourseSlide now carries course_language so the session can sync.
...
class CourseSlide:
    ...
    FIX-1: Added `course_language` field so the session can automatically
    sync its language to the course language when a presentation starts.
    """
    course_id: str
    course_title: str = ""
    course_domain: str = ""
    # FIX-1: language stored here so session_service can sync
    course_language: str = "fr"
    chapter_index: int = 0
    chapter_number: int = 1
    chapter_title: str = ""
    section_index: int = 0
