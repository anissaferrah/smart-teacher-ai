...
Γòæ        SMART TEACHER ΓÇö WebSocket Server (Streaming Audio)          Γòæ
Γòæ                                                                      Γòæ
Γòæ  Ce fichier est le point d'entr├⌐e PRINCIPAL du projet.              Γòæ
Γòæ  Il remplace server.py pour la production.                          Γòæ
Γòæ                                                                      Γòæ
Γòæ  Routes WebSocket :                                                  Γòæ
Γòæ    WS  /ws/{session_id}    ΓÇö pipeline vocal temps r├⌐el              Γòæ
Γòæ                                                                      Γòæ
Γòæ  Routes REST :                                                        Γòæ
Γòæ    POST /session            ΓÇö cr├⌐er une session avec token auth     Γòæ
Γòæ    POST /ask                ΓÇö texte ΓåÆ r├⌐ponse                       Γòæ
Γòæ    POST /ingest             ΓÇö indexer des fichiers dans le RAG      Γòæ
Γòæ    GET  /rag/stats          ΓÇö statistiques RAG                      Γòæ
Γòæ    GET  /session/{id}       ΓÇö ├⌐tat de la session                    Γòæ
Γòæ    GET  /health             ΓÇö healthcheck complet                   Γòæ
Γòæ                                                                      Γòæ
Γòæ  Messages WebSocket (client ΓåÆ serveur) :                            Γòæ
Γòæ    {"type": "start_session",  "token": "...", "language": "fr"}   Γòæ
Γòæ    {"type": "audio_chunk",    "data": "<base64>"}                   Γòæ
Γòæ    {"type": "audio_end"}                                            Γòæ
Γòæ    {"type": "interrupt"}                                            Γòæ
Γòæ    {"type": "next_section"}                                         Γòæ
Γòæ    {"type": "text",           "content": "..."}                     Γòæ
Γòæ    {"type": "quiz"}                                                Γòæ
Γòæ                                                                      Γòæ
Γòæ  Messages WebSocket (serveur ΓåÆ client) :                            Γòæ
Γòæ    {"type": "session_ready",  "session_id": "..."}                  Γòæ
Γòæ    {"type": "transcription",  "text": "...", "lang": "fr"}          Γòæ
Γòæ    {"type": "answer_text",    "text": "..."}                        Γòæ
Γòæ    {"type": "audio_chunk",    "data": "<base64>", "mime": "..."}    Γòæ
Γòæ    {"type": "state_change",   "state": "PROCESSING"}                Γòæ
Γòæ    {"type": "error",          "message": "..."}                     Γòæ
Γòæ    {"type": "quiz_prompt",    "quiz": {...}}                        Γòæ
...
log.info("=" * 70)
log.info("≡ƒñû SMART TEACHER ΓÇö D├ëMARRAGE (WebSocket + REST)")
log.info("=" * 70)
log.info("")
log.info("≡ƒôª Services Docker requis:")
log.info("   ΓÇó PostgreSQL (5432) ΓÇö Base de donn├⌐es")
log.info("   ΓÇó Redis (6379) ΓÇö Cache et sessions")
log.info("   ΓÇó Qdrant (6333) ΓÇö RAG vectoriel")
log.info("   ΓÇó Elasticsearch (9200) ΓÇö Recherche full-text historique")
log.info("   ΓÇó MinIO (9000) ΓÇö Stockage m├⌐dia objet")
log.info("   ΓÇó Ollama (11434) ΓÇö LLM local (fallback)")
log.info("")
log.info("≡ƒÆí Lancez-les: docker-compose up -d")
log.info("")
...
audio_input = AudioInput()  # Γ£à Silero VAD for backend voice detection
...
async def log_backend_diagnostics() -> None:
    ...
    log.info("≡ƒöÄ Diagnostic d├⌐marrage des services de donn├⌐es:")

    try:
        search_stats = transcript_searcher.get_stats()
        if search_stats.get("backend") == "elasticsearch":
            log.info("   ΓÇó Elasticsearch: Γ£à connect├⌐ (%s)", search_stats.get("host", "n/a"))
        else:
            log.info("   ΓÇó Elasticsearch: Γä╣∩╕Å recherche en m├⌐moire (fallback actif)")
    except Exception as exc:
        log.info("   ΓÇó Elasticsearch: Γä╣∩╕Å recherche en m├⌐moire (%s)", exc)

    try:
        storage_status = media_storage.get_status()
        if storage_status.get("provider") == "minio":
            log.info("   ΓÇó MinIO: Γ£à connect├⌐ (%s)", storage_status.get("endpoint", "n/a"))
        else:
            log.info("   ΓÇó MinIO: Γä╣∩╕Å stockage local (%s)", storage_status.get("local_root", "media"))
    except Exception as exc:
        log.info("   ΓÇó MinIO: Γä╣∩╕Å stockage local (%s)", exc)

    try:
        analytics_engine._init_ch()
        report = analytics_engine.full_report()
        if report.get("backend") == "clickhouse":
            log.info("   ΓÇó ClickHouse: Γ£à connect├⌐")
        else:
            log.info("   ΓÇó ClickHouse: Γä╣∩╕Å analytics CSV + m├⌐moire")
    except Exception as exc:
        log.info("   ΓÇó ClickHouse: Γä╣∩╕Å analytics CSV + m├⌐moire (%s)", exc)
    ...
    try:
        ...
        log.info("   ΓÇó Redis: Γ£à connect├⌐ (%s:%s)", redis_host, redis_port)
    except Exception as exc:
        log.info("   ΓÇó Redis: ΓÜá∩╕Å indisponible (%s)", exc)
...
#   - run_pipeline_streaming(audio, session_id, history, ...) ΓåÆ Streaming LLM+TTS
#   - run_pipeline(audio, session_id, history, ...) ΓåÆ Full audio pipeline
#
...
async def lifespan(app: FastAPI):
    """Initialisation l├⌐g├¿re au d├⌐marrage, avec d├⌐gradation si PostgreSQL est indisponible."""
    try:
        if await check_db_connection():
            await create_tables()
            log.info("Γ£à PostgreSQL tables cr├⌐├⌐es/valid├⌐es")
        else:
            log.info("Γä╣∩╕Å PostgreSQL indisponible au d├⌐marrage ΓÇö mode d├⌐grad├⌐ activ├⌐")
    except Exception as exc:
        log.info(f"Γä╣∩╕Å PostgreSQL non disponible au d├⌐marrage ({exc}) ΓÇö mode d├⌐grad├⌐ activ├⌐")
...
app = FastAPI(
    ...
    description="Professeur IA Vocal ΓÇö WebSocket + REST | STT+RAG+LLM+TTS",
    version="3.0.0",
    lifespan=lifespan,
)
...
# ≡ƒô╕ Routes pour les PNG slides (images visuelles du cours)
...
async def load_course_slide_context(
    ...
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            # Γ£à NOUVEAU: Analyser le cours pour le contexte LLM (une fois par cours)
            if chapter_index == 0:  # Premi├¿re fois = analyser le cours entier
                try:
                    ...
                    # ≡ƒô¥ Stocker pour utiliser plus tard
            ...
            return {
                ...
                # Γ£à NOUVEAU: Ajouter analysis du cours
...
# ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
#  WEBSOCKET ΓÇö Pipeline Vocal Temps R├⌐el
# ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    G├¿re le cycle complet : pr├⌐sentation ΓåÆ ├⌐coute ΓåÆ r├⌐ponse ΓåÆ reprise.
    """
    await websocket.accept()
    log.info(f"≡ƒöî WebSocket connect├⌐ : {session_id[:8]}")

    ctx: SessionContext | None = None
    audio_buffer: list[bytes] = []
    history: list[dict]       = []
    session_lang:  str         = "fr"
    session_level: str         = "lyc├⌐e"
    saved_chi:     int         = 0      # position chapitre sauvegard├⌐e
    saved_si:      int         = 0      # position section sauvegard├⌐e
    in_course:     bool        = False  # ├⌐tudiant en cours de pr├⌐sentation
    ...
    student_profile: dict = {}  # Γ£à Profil de l'├⌐tudiant pour timing adaptatif
    interrupt_audio: bool = False  # ≡ƒÜ¿ Flag pour interrompre le TTS en temps r├⌐el
    presentation_start_time: float = 0.0  # Γ£à NOUVEAU: Timestamp quand pr├⌐sentation commence
    ...
    async def send(data: dict):
        ...
        async with send_lock:
            ...
            try:
                ...
                log.info(f"[{session_id[:8]}] WebSocket ferm├⌐ pendant send: {exc}")
                return False

    async def handle_post_response(timeout_sec: float = 2.5) -> bool:
        """
        Apr├¿s que le LLM a r├⌐pondu.
        Attire interruption utilisateur (VAD).
        Timeout adaptatif selon le profil ├⌐tudiant (niveau + performance).
        ...
        try:
            # Γ£à Calculer timeout ADAPTATIF selon profil
            BASE_TIMING = {
                "coll├¿ge": 5.0,
                "lyc├⌐e": 4.0,
                "licence": 3.5,
            ...
            # R├⌐cup├⌐rer le timing de base selon le niveau
            base_timeout = BASE_TIMING.get(session_level, 3.0)
            
            # Γ£à Ajuster selon la confusion de l'├⌐tudiant
            if student_profile:
                confusion_count = student_profile.get("confusion_count", 0)
                asks_repeat = student_profile.get("asks_repeat", 0)
                
                # Plus confusion ΓåÆ plus de temps (ajouter 0.5s par confusion)
                confusion_factor = 1.0 + (confusion_count * 0.5)
                
                # Plus demande de r├⌐p├⌐titions ΓåÆ plus de temps
                ...
                log.info(
                    f"ΓÅ▒∩╕Å  Timing adaptatif | level={session_level} base={base_timeout:.1f}s "
                    f"confusion_factor={confusion_factor:.2f} repeat_factor={repeat_factor:.2f} "
                    f"ΓåÆ final={timeout_sec:.2f}s"
                )
            else:
                timeout_sec = base_timeout
            
            # ΓÅ╕∩╕Å  Transition vers WAITING (court repos)
            await dialogue.transition(ctx.session_id, DialogState.WAITING)
            await send_state(DialogState.WAITING)
            log.info(f"ΓÅ╕∩╕Å  [{session_id[:8]}] Waiting for interruption ({timeout_sec:.1f}s)...")
            
            # ΓÅ▒∩╕Å  Attendre interruption utilisateur (dur├⌐e timeout)
            start_wait = time.time()
            while time.time() - start_wait < timeout_sec:
                # V├⌐rifier si VAD a d├⌐tect├⌐ la parole
                session = await dialogue.get_session(ctx.session_id)
                if session and session.state == DialogState.LISTENING.value:
                    # Γ£à Utilisateur a interrompu!
                    log.info(f"≡ƒÄñ [{session_id[:8]}] User interrupt detected ΓåÆ LISTENING")
                    return True
                
                await asyncio.sleep(0.05)  # V├⌐rifier tous les 50ms
            
            # ΓÅ¡∩╕Å  Timeout ├⌐coul├⌐ = pas d'interruption ΓåÆ continuer auto
            log.info(f"Γû╢∩╕Å  [{session_id[:8]}] No interrupt ΓåÆ auto-advancing")
            return False
            
        except Exception as e:
            log.error(f"Γ¥î Error in handle_post_response: {e}")
            return False

    async def send_state(state: DialogState, substep: str = "", details: dict = None, metrics: dict = None, turn_id: int | None = None):
        """
        Γ£à ULTIME v2: ├ëtat ENRICHI COMPLET avec tous les sous-├⌐tats et m├⌐triques temps r├⌐el.
        
        Livre un message format├⌐ parfait avec:
        - ├ëtat principal (8 ├⌐tats)
        - Sous-├⌐tape TR├êS d├⌐taill├⌐e (16+ substeps)
        ...
        # DICTIONNAIRE COMPLET DES 16+ SOUS-├ëTAPES
        # ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
        substep_full = {
            # Audio Capture
            "stt_language_detection": {"emoji": "≡ƒîì", "step": "Language Detection", "desc": "Language identification", "detail": "(FR/EN auto-detect)"},
            "prosody_analysis": {"emoji": "≡ƒôè", "step": "Prosody Analysis", "desc": "Speech rate, hesitations, intonation", "detail": "(Emotion & confusion detection)"},
            
            # RAG Phase
            "rag_search": {"emoji": "≡ƒöì", "step": "Document Search", "desc": "Vector search + BM25", "detail": "(Course documents)"},
            "rag_ranking": {"emoji": "≡ƒôæ", "step": "Ranking Results", "desc": "Sort by relevance", "detail": "(Calculated score)"},
            
            # Confusion Detection
            "confusion_analyzing": {"emoji": "≡ƒºá", "step": "Confusion Analysis", "desc": "Comprehension check", "detail": "(8-level detection)"},
            "confusion_detected": {"emoji": "≡ƒñö", "step": "CONFUSION DETECTED", "desc": "LLM prompt adaptation", "detail": "(Reformulation needed)"},
            "confusion_none": {"emoji": "Γ£à", "step": "No Confusion", "desc": "Clear question", "detail": "(Standard prompt)"},
            
            # Semantic Checking
            "semantic_check": {"emoji": "≡ƒöù", "step": "Semantic Check", "desc": "Historical comparison", "detail": "(OpenAI similarity>85%)"},
            "semantic_repeat": {"emoji": "≡ƒöä", "step": "Similar Question", "desc": "Repetition detected", "detail": "(Different angle proposed)"},
            
            # LLM Processing
            "llm_thinking": {"emoji": "≡ƒºá", "step": "LLM Thinking", "desc": "Response generation", "detail": "(OpenAI/Mistral in progress)"},
            "llm_generating": {"emoji": "Γ£ì∩╕Å", "step": "Generating Response", "desc": "Text construction", "detail": "(Tokens generated)"},
            
            # Streaming Phase
            "streaming_llm": {"emoji": "≡ƒôí", "step": "LLM Streaming", "desc": "Direct transmission", "detail": "(Complete response)"},
            "tts_generating": {"emoji": "≡ƒÄÖ∩╕Å", "step": "Audio Generation", "desc": "Text-to-speech conversion", "detail": "(Edge TTS in progress)"},
            "tts_streaming": {"emoji": "≡ƒôó", "step": "Audio Streaming", "desc": "Audio playback", "detail": "(Direct chunks)"},
            
            # Completion
            "response_complete": {"emoji": "≡ƒÄë", "step": "Response Complete", "desc": "Sent successfully", "detail": "(Awaiting feedback)"},
            "feedback_listening": {"emoji": "≡ƒæé", "step": "Voice Feedback", "desc": "Say YES or REPEAT", "detail": "(2-3 seconds)"},
        }
        ...
        # ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
        state_map = {
            DialogState.IDLE: {"emoji": "≡ƒ¢æ", "name": "Idle", "description": "No active session"},
            DialogState.INDEXING: {"emoji": "≡ƒôÜ", "name": "Indexing", "description": "Course ingestion in progress..."},
            DialogState.PRESENTING: {"emoji": "≡ƒÄô", "name": "Presenting", "description": "AI presenting content"},
            DialogState.LISTENING: {"emoji": "≡ƒæé", "name": "Listening", "description": "AI listening to your question..."},
            DialogState.PROCESSING: {"emoji": "ΓÜÖ∩╕Å", "name": "Processing", "description": "Analysis and processing..."},
            DialogState.RESPONDING: {"emoji": "≡ƒùú∩╕Å", "name": "Responding", "description": "AI generating response..."},
            DialogState.WAITING: {"emoji": "ΓÅ│", "name": "Waiting", "description": "Awaiting feedback..."},
            DialogState.CLARIFICATION: {"emoji": "Γ¥ô", "name": "Clarification", "description": "Clarification needed..."},
        }
        
        state_info = state_map.get(state, {})
        emoji = state_info.get("emoji", "Γ¥ô")
        ...
        # Γ£à SHOW SUBSTEPS FOR STREAMING REASONING DISPLAY
        if substep and substep in substep_full:
            ...
            msg_lines.append(f"   ΓåÆ {sub['desc']}")
            if sub.get('detail'):
                msg_lines.append(f"   ≡ƒÆ¡ {sub['detail']}")
        
        # Additional details
        if details and details.get("reason"):
            msg_lines.append(f"   ≡ƒÄ» Reason: {details['reason'].replace('_', ' ').title()}")

        if details:
            detail_items = [
                ("course_title", "≡ƒôÿ Course"),
                ("chapter_title", "≡ƒôÜ Chapter"),
                ("section_title", "≡ƒöû Section"),
                ("slide_title", "≡ƒû╝∩╕Å Slide"),
                ("question_text", "Γ¥ô Question"),
                ("transcription", "≡ƒÄñ STT"),
                ("engine", "≡ƒ¢á∩╕Å Engine"),
                ("voice", "≡ƒÄÖ∩╕Å Voice"),
                ("tts_engine", "≡ƒùú∩╕Å TTS Engine"),
                ("tts_voice", "≡ƒÄÖ∩╕Å TTS Voice"),
                ("answer_preview", "≡ƒÆ¼ Answer"),
            ]
            for key, label in detail_items:
                ...
                if len(value_text) > 160:
                    value_text = value_text[:160].rstrip() + "ΓÇª"
                msg_lines.append(f"   {label}: {value_text}")
        
        # ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
        # REAL-TIME METRICS SECTION ≡ƒôè
        # Γ£à ONLY SHOW METRICS FOR RESPONDING STATE (end of processing)
        # ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
        if metrics and len(metrics) > 0 and state == DialogState.RESPONDING:
            msg_lines.append("")  # Empty line
            msg_lines.append("≡ƒôè Complete Metrics:")
            
            # Duration metrics
            if "elapsed" in metrics:
                msg_lines.append(f"   ΓÅ▒∩╕Å  Elapsed: {metrics['elapsed']:.2f}s")
            if "total_time" in metrics:
                msg_lines.append(f"   ΓÅ▒∩╕Å  Total: {metrics['total_time']:.2f}s")
            
            # Quality metrics
            if "confidence" in metrics:
                conf = metrics["confidence"]
                bar = "Γûô" * int(conf * 10) + "Γûæ" * (10 - int(conf * 10))
                msg_lines.append(f"   ≡ƒÄ» Confidence: {bar} {conf:.0%}")
            
            # STT metrics
            if "speech_rate" in metrics:
                msg_lines.append(f"   ≡ƒÄñ Speech Rate: {metrics['speech_rate']:.0f} wpm")
            if "hesitations" in metrics:
                msg_lines.append(f"   ≡ƒÆ¡ Hesitations: {metrics['hesitations']} found")
            if "language" in metrics:
                msg_lines.append(f"   ≡ƒîì Language: {metrics['language'].upper()}")
            
            # RAG metrics
            if "chunks" in metrics:
                msg_lines.append(f"   ≡ƒôÜ Documents: {metrics['chunks']} found")
            if "retrieval_time" in metrics:
                msg_lines.append(f"   ≡ƒöì Retrieval: {metrics['retrieval_time']:.3f}s")
            if "document_score" in metrics:
                msg_lines.append(f"   ≡ƒÄ▓ Top Score: {metrics['document_score']:.2f}/1.00")
            
            # Confusion metrics
            if "confusion_confidence" in metrics:
                conf = metrics["confusion_confidence"]
                msg_lines.append(f"   ≡ƒñö Confusion Level: {conf:.0%}")
            
            # LLM metrics
            if "tokens" in metrics:
                msg_lines.append(f"   ≡ƒöñ Tokens: {metrics['tokens']} generated")
            if "sentences" in metrics:
                msg_lines.append(f"   ≡ƒô¥ Sentences: {metrics['sentences']}")
            if "words" in metrics:
                msg_lines.append(f"   ≡ƒô░ Words: {metrics['words']}")
            if "llm_time" in metrics:
                msg_lines.append(f"   ≡ƒºá LLM Time: {metrics['llm_time']:.2f}s")
            
            # TTS metrics
            if "tts_chunks" in metrics:
                msg_lines.append(f"   ≡ƒÄÖ∩╕Å  Audio Chunks: {metrics['tts_chunks']}")
            if "audio_duration" in metrics:
                msg_lines.append(f"   ≡ƒöë Audio Duration: {metrics['audio_duration']:.1f}s")
            
            # Progress bar
            if "progress" in metrics:
                prog = metrics["progress"]
                bar = "Γûô" * int(prog * 12) + "Γûæ" * (12 - int(prog * 12))
    ...
    async def process_text_question_turn(
        *,
        ...
        try:
            ...
            # Γ£à STREAMING LLM WITH REAL-TIME METRICS
            ai_response = ""
            llm_confidence = 0.7
            chunk_count = 0
            tokens_generated = 0
            ...
            try:
                ...
                log.warning(
                    f"[{session_id[:8]}] ΓÜá∩╕Å  RAG failed ({type(rag_exc).__name__}): {str(rag_exc)[:100]} ΓåÆ Fallback brain.ask()..."
                )
                try:
                    ...
                    log.error(f"[{session_id[:8]}] Γ¥î Fallback also failed: {fallback_exc}")
                    ai_response = "Je suis d├⌐sol├⌐, j'ai rencontr├⌐ une erreur technique. Veuillez r├⌐essayer."
                    llm_confidence = 0.0
            ...
            # Γ£à UPDATE STATE: TTS Generation
            tts_start = time.time()
            ...
            # ΓöÇΓöÇ Analytics & Recherche ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            try:
    ...
    async def set_listening_state() -> None:
        """
        Transition S├ëCURIS├ëE vers LISTENING avec gestion de la machine d'├⌐tat.
        
        Γ£à Si en LISTENING: pas de transition n├⌐cessaire
        Γ£à Si en PROCESSING: passer par CLARIFICATION d'abord (transition valide)
        Γ£à Si dans autre ├⌐tat: transition directe
        """
        nonlocal ctx
        ...
        # Γ£à D├⌐j├á en LISTENING: rien ├á faire
        if current_state == DialogState.LISTENING.value:
            return
        
        # Γ£à Si en PROCESSING: passer par CLARIFICATION (transition valide: PROCESSING ΓåÆ CLARIFICATION)
        if current_state == DialogState.PROCESSING.value:
            log.info(f"[{session_id[:8]}] ≡ƒöä PROCESSING ΓåÆ CLARIFICATION ΓåÆ LISTENING (transition s├⌐curis├⌐e)")
            await dialogue.transition(ctx.session_id, DialogState.CLARIFICATION)
            await send_state(DialogState.CLARIFICATION)
            await send({"type": "message", "text": "Je n'ai rien entendu. Peux-tu r├⌐p├⌐ter?"})
        
        # Γ£à Si en PRESENTING: annuler d'abord la pr├⌐sentation
        if current_state == DialogState.PRESENTING.value:
            log.info(f"[{session_id[:8]}] ≡ƒ¢æ PRESENTING ΓåÆ LISTENING (annulation pr├⌐sentation)")
            await cancel_presentation_task(notify_client=False)
        
        # Γ£à Maintenant transition s├⌐curis├⌐e vers LISTENING
    ...
    async def record_pause_point(reason: str, notice_prefix: str = "ΓÅ╕ Point d'arr├¬t m├⌐moris├⌐") -> str:
        nonlocal ctx
        ...
        point_text = f"{notice_prefix} ΓÇö {location_label}, position {cursor_label}. Narration gard├⌐e en cache."
    ...
    async def prefetch_next_slide(
        ...
        log.info(
            f"[{session_id[:8]}] Γ£à Next slide prefetched | key={next_slide_id} | chars={len(next_narration_text)}"
        )
    ...
    async def synthesize_cached_tts(
        ...
        *,
        ...
        if cached and cached.get("audio_bytes"):
            log.info(
                f"[{session_id[:8]}] ΓÖ╗∩╕Å TTS cache hit | scope={cache_scope} | "
    ...
    async def _stream_audio(audio_bytes: bytes, mime: str | None, stream_id: int) -> None:
        ...
        for i in range(0, total_len, chunk_size):
            ...
            if not await send({
                ...
                return
            # Laisse la boucle ├⌐v├⌐nementielle respirer entre chunks
    ...
    async def explain_slide_focused(
        ...
        section_title: str = "",
        language: str = "fr",
        student_level: str = "lyc├⌐e",
        course_summary: str = "",  # Γ£à NOUVEAU: contexte du cours entier
        is_resume: bool = False,   # Γ£à NOUVEAU: d├⌐tecter pause/reprise
        session_id: str | None = None,  # Γ£à For rate limiting
    ) -> str:
        """
        Expliquer un slide de mani├¿re PR├ëCISE.
        ...
        Args:
            ...
            language: fr/en
            student_level: coll├¿ge/lyc├⌐e/licence/master/doctorat
            course_summary: R├⌐sum├⌐ du cours entier (contexte global)
            is_resume: True si pause/reprise (ne pas red├⌐marrer depuis d├⌐but)
        """
        try:
            # Γ£à NO RAG! JUSTE la slide actuelle + positionnement
            position_info = f"Chapitre {chapter_idx + 1}: {chapter_title}"
            if section_title:
                position_info += f" ΓåÆ Section: {section_title}"
            
            # Γ£à Contexte global du cours (pour coh├⌐rence)
            course_context = course_summary or position_info
            
            # Γ£à Adapter le ton selon le niveau
            level_instruction = {
                "coll├¿ge": "Explique tr├¿s simplement, avec des exemples concrets et accessibles. ├ëvite la terminologie complexe.",
                "lyc├⌐e": "Sois clair et structur├⌐. Utilise la terminologie exacte mais reste accessible.",
                "licence": "Sois technique et pr├⌐cis. Utilise la terminologie universitaire. Cite les concepts cl├⌐s.",
                "master": "Utilise un langage technique avanc├⌐. Analyse critique. R├⌐f├⌐rences th├⌐oriques attendues.",
                "doctorat": "Analyse approfondie avec terminologie sp├⌐cialis├⌐e. Enjeux de recherche. Nuances critiques.",
            }
            level_hint = level_instruction.get(student_level, level_instruction["lyc├⌐e"])
            
            # Γ£à Prompt ULTRA STRICT anti-hallucination (TR├êS IMPORTANT)
            if language == "fr":
                if is_resume:
                    # ≡ƒöä Mode REPRISE : ne pas red├⌐marrer depuis le d├⌐but
                    prompt_mode = """ATTENTION: Cette slide a D├ëJ├Ç ├⌐t├⌐ expliqu├⌐e.
NE RECOMMENCE PAS L'EXPLICATION.
Continues o├╣ tu t'├⌐tais arr├¬t├⌐ ou propose des d├⌐tails suppl├⌐mentaires."""
                else:
                    prompt_mode = ""
                
                system_prompt = f"""Tu es Smart Teacher, expert p├⌐dagogue.

**R├êGLES ABSOLUES (non-n├⌐gociable)**:

1. Γ¥î JAMAIS parler de :
   - Chapitres futurs
   - Sujets "qui viennent apr├¿s"
...
3. ≡ƒÜ½ Si tu ne sais pas :
   - NE PAS inventer
   - Dis: "Ce d├⌐tail n'est pas expliqu├⌐"
...
Explique ├ºa naturellement. MAX 4 phrases."""
...
Explain naturally. MAX 4 sentences."""
            ...
            # Γ£à LLM call direct (pas de RAG, juste la slide)
            response, duration = await asyncio.to_thread(
                brain.ask,
                question=slide_content[:100],  # Juste premi├¿re partie pour r├⌐f├⌐rence
                ...
                domain=None,
                session_id=session_id,  # Γ£à Pass session_id for rate limiting
            )
            
            log.info(f"Γ£à LLM focused response ({duration:.1f}s) | {len(response)} chars")
            return response
            
        except Exception as e:
            log.error(f"Γ¥î Error in explain_slide_focused: {e}")
    ...
    try:
        while True:
            ...
            # ΓöÇΓöÇ start_session ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            if msg_type == "start_session":
                # Γ£à Flexible auth: Only validate if we have a token entry
                ...
                if session_id in SESSION_TOKENS:
                    ...
                    if not token or SESSION_TOKENS.get(session_id) != token:
                        ...
                        log.warning(f"[{session_id[:8]}] Γ¥î WebSocket auth failed (invalid token)")
                        return
                    # Token is valid - consume it (one-time use)
                    del SESSION_TOKENS[session_id]
                    log.info(f"[{session_id[:8]}] Γ£à WebSocket authenticated (secure token)")
                else:
                    # Legacy flow: No token generated, so ignore any token and auto-approve
                    log.info(f"[{session_id[:8]}] Γ£à WebSocket auto-approved (legacy SDK)")
                
                lang  = msg.get("language", "fr")
                level = msg.get("level",    "lyc├⌐e")
                ...
                session_level = level
                history.clear()
                interrupt_audio = False  # ≡ƒƒó R├⌐initialiser le flag
                await cancel_text_question_task()
                active_text_turn_id = 0
                text_turn_seq = 0
                ...
                try:
                    ...
                    log.warning(f"[{session_id[:8]}] ΓÜá∩╕Å Learning session DB init failed: {exc}")

                await send({"type": "session_ready", "session_id": ctx.session_id})
                
                # Γ£à NOUVEAU: Transition explicite IDLE ΓåÆ LISTENING
                await dialogue.transition(ctx.session_id, DialogState.LISTENING)
                await send_state(DialogState.LISTENING)
                log.info(f"[{session_id[:8]}] ≡ƒÜÇ Session d├⌐marr├⌐e | lang={lang} level={level} state=LISTENING")

            # ΓöÇΓöÇ audio_chunk ΓÇö accumule les donn├⌐es audio ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                ...
                if raw:
                    try:
                        ...
                        log.info(f"   ≡ƒôª Audio chunk: base64_len={len(raw)} ΓåÆ {len(decoded)} bytes")
                        # Γ£à REFRESH ctx (peut ├¬tre modifi├⌐ par run_presentation)
                        if ctx:
                        ...
                        if ctx and ctx.state in [DialogState.RESPONDING.value, DialogState.PRESENTING.value]:
                            ...
                            # Γ£à NOUVEAU: D├⌐terminer si interruption est TR├êS PR├ëCOCE (avant streaming)
                            ...
                            if ctx.state == DialogState.PRESENTING.value and time_since_presenting < 1.0:
                                log.info(f"[{session_id[:8]}] ΓÜí TR├êS T├öT interruption ({time_since_presenting:.2f}s apr├¿s PRESENTING) ΓåÆ annuler pr├⌐paration")
                            
                            # Γ£à CRUCIAL: Sauvegarder le point AVANT d'annuler
                            await record_pause_point("voice_interrupt", notice_prefix="ΓÅ╕ Point d'arr├¬t m├⌐moris├⌐ (voix)")
                            
                            # Γ£à Annuler la pr├⌐sentation EN COURS pour ├⌐viter conflit
                        ...
                        log.error(f"Γ¥î Failed to decode audio chunk: {e}")

            # ΓöÇΓöÇ audio_end ΓÇö traite l'audio accumul├⌐ ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            elif msg_type == "audio_end":
                interrupt_audio = False
                turn_id = int(msg.get("turn_id") or 0)
                
                # Γ£à REFRESH ctx FROM REDIS (peut ├¬tre modifi├⌐ par run_presentation en t├óche async)
                if not ctx:
                    ctx = await dialogue.get_session(session_id)
                else:
                    ctx = await dialogue.get_session(session_id) or ctx
                
                # Γ£à S├ëCURIT├ë: Ne jamais traiter audio_end sans session active
                if not ctx:
                    await send({"type": "error", "message": "Aucune session active (d├⌐marrez avec start_session)", "turn_id": turn_id})
                    continue
                
                # Γ£à S├ëCURIT├ë: Bloquer transition IDLE ΓåÆ PROCESSING (mais LISTENING OK!)
                if ctx.state == DialogState.IDLE.value:
                    log.warning(f"[{session_id[:8]}] ΓÜá∩╕Å Tentative audio_end en IDLE ΓåÆ ignor├⌐ (jamais IDLE ΓåÆ PROCESSING)")
                    await send({"type": "error", "message": "Session en IDLE, d├⌐marrage n├⌐cessaire", "turn_id": turn_id})
                    continue
                
                if not audio_buffer:
                    await send({"type": "error", "message": "Aucun audio re├ºu", "turn_id": turn_id})
                    continue
                await cancel_audio_stream(notify_client=False)

                # Assembler et convertir l'audio
                full_audio = b"".join(audio_buffer)
                log.info(f"[{session_id[:8]}] ≡ƒÄÖ∩╕Å Audio buffer assembled: {len(audio_buffer)} chunks = {len(full_audio)} total bytes")
                audio_buffer.clear()
                ...
                # ≡ƒÄñ SILERO VAD: Filter audio with backend voice detection (PyTorch)
                # ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
                log.info(f"[{session_id[:8]}] ≡ƒÄñ Silero VAD: Filtering {len(audio_np)} samples...")
                ...
                if filtered_chunks:
                    ...
                    log.info(f"[{session_id[:8]}] Γä╣∩╕Å Silero VAD: No speech detected (all chunks < threshold)")
                    await send({
                        "type": "system_notice",
                        "text": "Aucune voix d├⌐tect├⌐e.",
                        "turn_id": turn_id,
                    })
                    await dialogue.transition(ctx.session_id, DialogState.LISTENING)
                    await send_state(DialogState.LISTENING, turn_id=turn_id)
                ...
                # Transition ΓåÆ PROCESSING (s├⌐curis├⌐e via state machine)
                ...
                # ≡ƒÜÇ STREAMING PIPELINE: Real-time LLM ΓåÆ TTS
                # ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
                ...
                async def on_transcription(text: str, lang: str, confidence: float):
                    ...
                    if not await send({
                        ...
                        log.warning(f"[{session_id[:8]}] ΓÜá∩╕Å Transcription non envoy├⌐e")
                
                async def on_state_change(substep: str, details: dict = None):
                    """Γ£à NOUVEAU: Callback pour les mises ├á jour d'├⌐tat de la pipeline"""
                ...
                async def on_audio_chunk(audio_bytes: bytes, mime: str):
                    ...
                    if audio_bytes:
                        ...
                        total_len = len(audio_bytes)
                        for i in range(0, total_len, chunk_size):
                            # ≡ƒÜ¿ V├⌐rifier le flag d'interruption EN TEMPS R├ëEL
                            if interrupt_audio:
                                log.info(f"[{session_id[:8]}] ≡ƒ¢æ Audio interruption d├⌐tect├⌐e ΓåÆ STOP streaming")
                                return
                ...
                try:
                    ...
                    course_id_for_rag = ctx.course_id if ctx else None  # Γ£à Pass course_id from session context
                    result = await run_pipeline_streaming(
                        audio_np, session_id, history,
                        on_text_chunk=on_text_chunk,
                        on_transcription=on_transcription,
                        on_audio_chunk=on_audio_chunk,
                        on_state_change=on_state_change,  # Γ£à NOUVEAU: State updates
                        force_language=course_lang,
                        course_id=course_id_for_rag,  # Γ£à Scoped RAG retrieval
                        ctx=ctx,
                        # Γ£à Inject dependencies
                        transcriber=transcriber,
                        rag=rag,
                        voice=voice,
                        brain=brain,
                    ...
                    if result.get("no_speech"):
                        await send({
                            "type": "system_notice",
                            "text": result.get("message", "Aucune voix d├⌐tect├⌐e."),
                            "turn_id": turn_id,
                        })
                        await save_media_json(
                            f"turns/transcripts/{session_id}/{turn_id}_{media_stamp}.json",
                            {
                                ...
                                "audio_answer_path": "",
                                "note": result.get("message", "Aucune voix d├⌐tect├⌐e."),
                            },
                        )
                        await dialogue.transition(ctx.session_id, DialogState.LISTENING)
                        await send_state(DialogState.LISTENING, turn_id=turn_id)
                    ...
                    if "error" in result:
                        # Γ£à Quand STT ├⌐choue: passer par CLARIFICATION (transition valide depuis PROCESSING)
                        ...
                        # Puis revenir ├á LISTENING via set_listening_state()
                        ctx = await dialogue.get_session(session_id) or ctx  # Γ£à Refresh ctx
                        await set_listening_state()
                        continue

                    # Transition ΓåÆ RESPONDING (now streaming)
                    if ctx:
                    ...
                    # Γ£à CRITICAL: Attendre interruption ou auto-avancer
                    await asyncio.sleep(0.5)  # Petit pause apr├¿s TTS
                    
                    user_interrupted = await handle_post_response(timeout_sec=2.5)
                    
                    if user_interrupted:
                        # Γ£à Utilisateur a pos├⌐ une question ΓåÆ ├⌐couter
                        log.info(f"≡ƒÆ¼ [{session_id[:8]}] Student asked a question")
                        # State est d├⌐j├á LISTENING (detectable par VAD)
                    else:
                        # Γ£à Pas de question ΓåÆ auto-avancer PR├ëSENTATION
                        log.info(f"ΓÅ¡∩╕Å  [{session_id[:8]}] Auto-advancing to next slide")
                        if ctx:
                            await dialogue.transition(ctx.session_id, DialogState.PRESENTING)
                        await send({"type": "next_section", "turn_id": turn_id})
                        # Laisser le client g├⌐rer le chargement du slide suivant
            ...
            # ΓöÇΓöÇ interrupt ΓÇö l'├⌐tudiant coupe l'IA ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            elif msg_type == "interrupt":
                interrupt_reason = str(msg.get("reason") or "question").strip().lower() or "question"
                turn_id = int(msg.get("turn_id") or 0)
                await record_pause_point(interrupt_reason, notice_prefix="ΓÅ╕ Point d'arr├¬t m├⌐moris├⌐")
                await cancel_presentation_task(notify_client=True)
                audio_buffer.clear()
                await cancel_audio_stream(notify_client=True, turn_id=turn_id)
                await cancel_text_question_task()
                if ctx and interrupt_reason != "pause":
                    ...
                    await set_listening_state()
                log.info(f"[{session_id[:8]}] ΓÜí Interruption")

            # ΓöÇΓöÇ present_section ΓÇö pr├⌐senter une section de cours ΓöÇΓöÇΓöÇΓöÇΓöÇ
                ...
                if ctx:
                    # Γ£à Stocker le course_summary pour utilisation dans explain_slide_focused
                ...
                # Envoyer la slide imm├⌐diatement avec ses m├⌐tadonn├⌐es exactes.
                if not await send({
                    "type": "slide_update",
                ...
                # Profil ├⌐tudiant ΓåÆ adapter le d├⌐bit (TODO: implement speech rate customization)
                try:
                    profile = await profile_mgr.get_or_create(session_id, lang_ps, session_level)
                    # Γ£à Stocker profil pour timing adaptatif
                ...
                async def run_presentation(
                    ...
                    try:
                        ...
                        if not narration_text:
                            ...
                            try:
                                # Γ£à D├⌐terminer si c'est une reprise (pause/resume)
                                is_resuming = resume_offset > 0 and reuse_cached

                                # Γ£à R├⌐cup├⌐rer le course_summary depuis le contexte session
                                course_summary = ctx.course_summary if ctx else ""

                                # Γ£à NOUVEAU: Utiliser explain_slide_focused (strict + focused + level-adapted)
                                narration_text = await explain_slide_focused(
                                    ...
                                    language=lang_ps,
                                    student_level=session_level,
                                    course_summary=course_summary,  # Γ£à NOUVEAU
                                    is_resume=is_resuming,  # Γ£à NOUVEAU
                                    session_id=session_id,  # Γ£à Pass session_id for rate limiting
                                )
                            except Exception:
                                # Fallback: brain.ask
                        ...
                        log.error(f"Erreur pr├⌐sentation streaming: {e}")
                        await send({"type": "error", "message": f"Erreur pr├⌐sentation: {str(e)}"})
                    finally:
                        nonlocal_presentation_task = presentation_task
                        if nonlocal_presentation_task is asyncio.current_task():
                            presentation_task = None

                # Γ£à NOUVEAU: Enregistrer le moment o├╣ la pr├⌐sentation COMMENCE
            ...
            # ΓöÇΓöÇ text ΓÇö question texte (depuis l'input HTML) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            elif msg_type == "text":
                # Γ£à REFRESH ctx (peut ├¬tre modifi├⌐ par run_presentation)
                if ctx:
                    ctx = await dialogue.get_session(session_id) or ctx
                ...
                if is_in_course and content_lower in resume_triggers:
                    ...
                    active_text_turn_id = 0
                    if ctx:
                        # Γ£à BUG #4 FIX: Retrieve char_offset from paused_state
                        ...
                        if resumed_ctx and resumed_ctx.paused_state.get("timestamp") is not None:
                            ...
                            log.info(f"≡ƒôì [{ctx.session_id[:8]}] Resume TTS from char {current_presentation_cursor}")
                            record_checkpoint_event({
                                ...
                                "point_text": (
                                    f"Γû╢ Reprise au point m├⌐moris├⌐ ΓÇö chapitre {resumed_ctx.chapter_index + 1}, "
                ...
                # Γ£à Priorit├⌐ ├á la question: stopper imm├⌐diatement la pr├⌐sentation en cours
                # pour ├⌐viter que la narration du cours continue pendant la r├⌐ponse.
                turn_id = int(msg.get("turn_id") or 0)
                if turn_id <= 0:
                    text_turn_seq += 1
                ...
                # Γ£à UPDATE STATE: Language Detection + Prosody (text: estimated)
                await send_state(
                    DialogState.PROCESSING, 
                ...
                # Γ£à S29-32: D├ëTECTION DE CONFUSION AUTOMATIQUE (g├⌐n├⌐ralise audio_pipeline)
                # Inclut: mots-cl├⌐s, r├⌐p├⌐tition, patterns d'historique + SEMANTIC (si brain fourni)
                # Pour questions texte, cr├⌐er prosody estim├⌐ (pas de voix r├⌐elle)
                text_prosody = {
                ...
                # Γ£à Helper function to emit micro-states during confusion detection
                ...
                is_confused, confusion_reason, q_hash, confusion_count = await dialogue.detect_and_track_confusion(
                    ...
                    question_text=content,
                    language=lang,
                    history=history,  # ΓåÉ Inclure l'historique pour pattern detection
                    brain=brain,      # ΓåÉ NOUVEAU: Embeddings s├⌐mantiques
                    prosody=text_prosody,  # ΓåÉ Pour questions texte: dummy/estim├⌐
                    on_state_change=emit_confusion_micro_state,  # Γ£à Pass callback for micro-states
                )
                
                # Γ£à UPDATE STATE: RAG Search
                rag_start = time.time()
                ...
                chunks_with_scores = rag.retrieve_chunks(
                    ...
                    current_chapter_idx=current_chapter_idx,
                    strict_chapter=bool(current_chapter_idx),
                    course_id=course_id if course_id else None,  # Γ£à Scoped RAG retrieval
                )
                ...
                # Γ£à UPDATE STATE: RAG Search Complete with Metrics
                ...
                # Γ£à UPDATE STATE: Confusion Detection (if any)
                if is_confused:
                ...
                if is_confused:
                    ...
                    log.info(f"[{session_id[:8]}] ≡ƒô¥ Prompt reformulation appliqu├⌐ au LLM (texte)")
                else:
            ...
            # ΓöÇΓöÇ quiz ΓÇö g├⌐n├⌐rer un quiz structur├⌐ ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            elif msg_type == "quiz":
                if ctx:
                    ctx = await dialogue.get_session(session_id) or ctx
            ...
            # ΓöÇΓöÇ next_section ΓÇö avancer dans le cours ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            elif msg_type == "next_section":
                if ctx:
                    await dialogue.next_section(ctx.session_id)
                await send({"type": "section_changed", "direction": "next"})

            # ΓöÇΓöÇ record dashboard event ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            # (done after each complete interaction)

            # ΓöÇΓöÇ ping / keepalive ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            elif msg_type == "ping":
                await send({"type": "pong"})

    except WebSocketDisconnect:
        websocket_closed = True
        log.info(f"≡ƒöî WebSocket d├⌐connect├⌐ : {session_id[:8]}")
    except Exception as exc:
        log.error(f"Γ¥î WebSocket error [{session_id[:8]}]: {exc}", exc_info=True)
...
# ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
#  ROUTES REST (compatibilit├⌐ + ingestion)
# ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
async def create_session():
    ...
    log.info(f"Γ£à Session created: {session_id[:8]} with auth token")
...
async def dashboard_services():
    ...
    services["rag"] = {
        ...
        "role": "Orchestre la recherche hybride et la g├⌐n├⌐ration de r├⌐ponses",
    ...
    redis_status.update({
        "role": "Cache et ├⌐tat temps r├⌐el",
        "retrieves": "Sessions WebSocket, ├⌐tat temporaire et latence de traitement",
    ...
    postgres_status.update({
        ...
        "retrieves": "Sessions, interactions, profils ├⌐tudiants et ├⌐v├⌐nements d'apprentissage",
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {
                    ...
                    "model": model_name,
                    "endpoint": endpoint,
                    "status": "Γ£à Ready" if available else "ΓÜá∩╕Å Model missing",
                    "models": model_names,
                }
            return {
                "available": False,
                "model": model_name,
                "endpoint": endpoint,
                "status": f"Γ¥î HTTP {response.status_code}",
            }
        except Exception as exc:
            return {
                "available": False,
    ...
    ollama_status.update({
        ...
        "retrieves": "Mod├¿les disponibles via /api/tags et r├⌐ponses locales via Ollama",
    ...
    services["elasticsearch"] = {
        ...
        "retrieves": "Questions, r├⌐ponses et index texte des transcriptions",
    ...
    services["minio"] = {
        ...
        "role": "Stockage objet des m├⌐dias",
        "retrieves": "PDF, slides, audio et objets list├⌐s via /media-list",
...
async def ask_question(request: Request, question: str = Form(...), course_id: str | None = Form(None)):
    ...
    # Γ£à course_id can be passed via form or headers
    if not course_id:
        course_id = request.headers.get("X-Course-ID")
    
    lang   = detect_lang_text(question)
    subj   = detect_subject(question)
    chunks_with_scores = rag.retrieve_chunks(question, k=Config.RAG_NUM_RESULTS, course_id=course_id)  # Γ£à Pass course_id
...
async def ingest_files(
    ...
    course_id:   str | None       = Form(None),  # Γ£à Allow specifying course_id for strict filtering
):
    """
    Upload et indexe des fichiers de cours dans la base vectorielle.
    
    - Utilise BAAI/bge-m3 par d├⌐faut pour l'ingestion locale
    - OpenAI reste disponible uniquement si configur├⌐ explicitement
    - Ingestion asynchrone track├⌐e par IngestionManager
    - course_id: UUID du cours (optionnel, pour filtrage strict par course)
    - Retour imm├⌐diat avec status
    """
    if not files:
    ...
    log.info(f"≡ƒôñ Ingestion lanc├⌐e ({len(saved_paths)} fichier(s)) ΓÇö embeddings locaux BAAI/bge-m3")
    if course_id:
        log.info(f"   ≡ƒôÜ course_id={course_id}")

    # Γ£¿ Lance l'ingestion en async et retour imm├⌐diat
    asyncio.create_task(
    ...
    return {
        ...
        "message": "Ingestion lanc├⌐e en arri├¿re-plan. Consultez /ingestion/status pour suivre la progression."
    }
...
async def _run_ingestion_background(
    ...
    domain: str = "general",
    course: str = "uploaded",
    course_id: str | None = None,  # Γ£à Add course_id parameter
) -> None:
    """
    Fonction interne : ingestion asynchrone avec IngestionManager.
    Lance l'ingestion en arri├¿re-plan et track la progression.
    """
    try:
        await ingestion_manager.start_ingestion(len(file_paths))
        
        # Γ£¿ Utilise la RAG principale (BAAI/bge-m3 par d├⌐faut en ingestion)
        ...
        ok = await loop.run_in_executor(
            ...
            lambda: rag.run_ingestion_pipeline_for_files(
                ...
                domain=domain,
                course=course,
                course_id=course_id,  # Γ£à Pass course_id to ingestion
                incremental=incremental,
            )
        )
...
async def get_ingestion_status():
    """Endpoint pour tracker l'├⌐tat d'ingestion en cours"""
...
async def debug_rag_test(q: str = "explain this topic", k: int = 5):
    """
    DEBUG endpoint: Teste le RAG et affiche ce qu'il retri├¿ve.
...
async def cache_stats():
    ...
    Returns:
        - cache_hits: Nombre de fois o├╣ le cache a retourn un embedding
        - cache_misses: Nombre de fois o├╣ il fallait calculer l'embedding
    ...
    return {
        ...
        "recommendations": {
            "note": "Cache hits > 60% = tr├¿s bon. < 30% = peut indiquer queries tr├¿s vari├⌐es.",
...
async def search_transcripts(q: str, language: str = "", course_id: str = "",
    ...
    if not q.strip():
        raise HTTPException(status_code=400, detail="Param├¿tre 'q' requis")
...
async def analytics_kpi(hours: int = 24):
    """KPIs sur les N derni├¿res heures."""
...
async def serve_media(path: str):
    """Sert les fichiers m├⌐dia stock├⌐s localement."""
...
async def reset_student_profile(student_id: str):
    """Remet ├á z├⌐ro le profil d'un ├⌐tudiant."""
...
async def build_course(
    ...
    level:    str              = Form("lyc├⌐e"),
    domain:   str              = Form("general"),  # ≡ƒÄ» Domaine (general, informatique, etc.)
):
    """
    Upload un PDF/DOCX ΓåÆ le structure en cours pr├⌐sentable.
    Sauvegarde dans PostgreSQL et RAG. G├⌐n├¿re les PNG slides.
    """
    ...
    for f in files:
        ...
        try:
            ...
            log.info(f"≡ƒôü Sauvegarde cours : {dest}")

            # 1. Construire le cours de fa├ºon directe, page par page
            ...
            try:
                ...
                db_error = str(exc)
                log.info(f"Γä╣∩╕Å PostgreSQL indisponible pour {f.filename}: {exc}")

            # Γ£à Store structured course data for indexing
            ...
            log.error(f"Γ¥î Build course failed for {f.filename}: {exc}")
    ...
    if files_to_index:
        log.info(f"≡ƒôñ Ingestion RAG batch lanc├⌐e pour {len(files_to_index)} fichier(s)")
        for item in files_to_index:
            ...
            log.info(f"   ≡ƒôÜ Structured indexing {Path(course_data.get('file_path', 'unknown')).name} ({target_domain}/{target_course}/{target_chapter}) with course_id={course_id}")
            ...
            if not rag_ok:
                log.info(f"Γä╣∩╕Å Ingestion RAG termin├⌐e sans indexation vectorielle pour {item['storage_path']}")
...
async def get_course_structure(course_id: str):
    """Retourne la structure compl├¿te d'un cours (chapitres + sections + PNG slides)."""
    try:
        from database.init_db import AsyncSessionLocal
        from database.crud import get_course_with_structure
        import uuid
        ...
        async with AsyncSessionLocal() as db:
            ...
            # ≡ƒÄ¿ R├⌐cup├⌐rer les slides depuis les sections (image_url)
            slides = []
            chapters_data = []
            ...
            for ch in course.chapters:
                ...
                for sec in ch.sections:
                    ...
                    sections_data.append({
                        ...
                        "image_url":  slide_path,  # ≡ƒÄ¿ PNG de cette slide
            ...
            return {
                ...
                "domain":   course.domain or "general",  # ≡ƒÄ» Domaine du cours
                "language": course.language,
                "level":    course.level,
                "file_path": course.file_path or "",
                "slides":   slides,  # ≡ƒÄ¿ Liste des PNG pour l'affichage
...
if __name__ == "__main__":
    ...
    try:
        ...
        log.error(
            "≡ƒÜ½ Port %s d├⌐j├á utilis├⌐ sur %s:%s (%s) ΓåÆ arr├¬tez l'autre instance ou changez SERVER_PORT",
            Config.SERVER_PORT,
            Config.SERVER_HOST,
            Config.SERVER_PORT,
            exc,
    ...
    log.info("")
    log.info("≡ƒîÉ Interface UI       : http://localhost:" + str(Config.SERVER_PORT) + "/static/index.html")
    log.info("≡ƒôÜ Swagger Docs       : http://localhost:" + str(Config.SERVER_PORT) + "/docs")
    log.info("≡ƒöî WebSocket tunnels  : ws://localhost:" + str(Config.SERVER_PORT) + "/ws/{session_id}")
    log.info("")
    log.info("Γ£à Serveur actif ΓÇö Appuyez sur Ctrl+C pour arr├¬ter")
    log.info("")
    uvicorn.run(app, host=Config.SERVER_HOST, port=Config.SERVER_PORT, reload=False)


# ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
#  ROUTES COURS ΓÇö Construction + Pr├⌐sentation
# ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
