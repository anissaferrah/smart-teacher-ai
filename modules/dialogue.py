...
VALID_TRANSITIONS: dict[DialogState, list[DialogState]] = {
    ...
    DialogState.PROCESSING: [DialogState.RESPONDING, DialogState.CLARIFICATION, DialogState.IDLE],
...
class DialogueManager:
    ...
    # ── Pause/Reprise ────────────────────────────────────────────────────────
    async def pause_session(self, session_id: str, slide_id: Optional[str] = None, char_offset: int = 0) -> Optional[SessionContext]:
        ...
        await self._save(ctx)
        log.info(f"⏸️  [{session_id[:8]}] Paused at offset {char_offset}")
        return ctx
    ...
    async def resume_session(self, session_id: str) -> Optional[SessionContext]:
        ...
        offset = ctx.paused_state.get("char_offset", 0)
    ...
    async def handle_interruption(self, session_id: str) -> Optional[SessionContext]:
        ...
        if ctx.state == DialogState.PRESENTING.value:
    ...
    def detect_confusion(self, text: str, language: str = "fr") -> tuple[bool, str]:
        ...
        Niveaux de détection :
          ...
          2. Mots-clés de confusion ("je comprends pas")
          3. Question trop courte + interrogative (< 4 mots + "?")
        """
        import re
        ...
        confusion_keywords = {
            ...
            "ar": [
                "ما فهمت", "لم أفهم", "ما فاهم", "مش فاهم",
                "اشرح", "أعد الشرح", "مرة ثانية",
                "ما هو", "ما هي", "كيف", "لماذا",
                "مش واضح", "محتاج مساعدة",
            ],
            "en": [
    ...
    def build_confusion_prompt(
        ...
        intros = {
            ...
            "ar": {
                "keyword": "الطالب لم يفهم. أعد الشرح بطريقة مختلفة مع مثال بسيط.",
                "repeated": "الطالب يكرر نفس السؤال. الشرح السابق لم يكن واضحاً. "
                            "جرّب أسلوباً مختلفاً تماماً مع تشبيه أو مثال من الحياة.",
                "too_short": "الطالب يبدو في حيرة. لخّص ما تم شرحه في جملتين، "
                             "ثم اسأله عن الجزء غير الواضح.",
                "pattern_short_burst": "الطالب يسأل أسئلة قصيرة كثيرة جداً. ارجع للأساسيات.",
                "pattern_repeated_concept": "الطالب يسأل نفس السؤال بطرق مختلفة. جرّب تشبيهاً أو مثالاً.",
                "pattern_too_many_questions": "الطالب يسأل أسئلة كثيرة جداً. قد يحتاج لاستراحة.",
            },
            "en": {
    ...
    async def check_semantic_repetition(
        ...
        """
        ✅ NOUVEAU (Couche D): Vérifie répétition sémantique via OpenAI embeddings.
        ...
        Coût: ~$0.0001 par appel (text-embedding-3-small).
        """
        if not brain or not brain.client or not question.strip():
            return False
        ...
        try:
            ...
            response = brain.client.embeddings.create(
    ...
    async def detect_and_track_confusion(
        ...
        2. Question répétée (repeated) — hash-based
        3. Question très courte (too_short)
        4. Patterns d'historique (pattern_*)
        5. Répétition sémantique (semantic) ← si brain fourni
        6. Marqueurs prosodiques (prosody_slow_speech, prosody_hesitations) ← NOUVEAU!
        7. Seuils adaptatifs par étudiant (Couche #1)
        ...
        if on_state_change:
            try:
                await on_state_change("confusion_keywords", {
                    "keywords_matched": 1 if is_confused and not reason else 0,
