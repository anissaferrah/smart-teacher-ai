...
"""
FIXED realtime_session_service.py
Addresses 6 critical bugs:
  1. Language sync: session.language = course.language at presentation start
  2. Q&A language: dynamically detect language from student message
  3. Chapter index: extracted from metadata, not defaulting to 0
  4. Section index: maps to PDF page number
  5. Slide ordering: strict sequential sort before presentation
  6. Student auth/profile: validated at session start

Changes are marked with # FIX-N comments.
"""
...
class RealtimeSessionService:
    ...
    # ------------------------------------------------------------------
    # FIX-2: Detect language from student's actual text
    # ------------------------------------------------------------------
    @staticmethod
    def _detect_language(text: str, fallback: str = "fr") -> str:
        """Detect language from text using langdetect, fallback to session default."""
        if not text or len(text.strip()) < 3:
            return fallback
        try:
            from langdetect import detect
            lang = detect(text)
            # Map langdetect codes to our 2-char codes
    ...
    async def handle(self, websocket: WebSocket, session_id: str) -> None:
        ...
        # ------------------------------------------------------------------
        # FIX-5: Slides sorted strictly sequentially by order/page_index
        # FIX-3: Chapter index extracted from DB metadata, not hardcoded
        # FIX-4: Section index = PDF page number from section.order
        # ------------------------------------------------------------------
        async def handle_presentation_turn(
            course_id: str, chapter_idx: int, section_idx: int
        ) -> None:
            nonlocal ctx
            ...
            try:
                ...
                # FIX-5: load_slide already uses sorted sections. We validate
                # that chapter_idx and section_idx are within bounds.
                ...
                # FIX-1: Sync session language to course language
                if slide.course_domain or hasattr(slide, 'course_language'):
                    course_lang = getattr(slide, 'course_language', None)
                    if course_lang and course_lang != ctx.language:
                        ctx.language = course_lang
                        log.info(
                            f"[{session_id[:8]}] FIX-1: session language synced to "
                            f"course language: {course_lang}"
                        )
                ...
                presentation_reasoning = [
                    {
                        ...
                        "status": "done",
                        "details": {
                            "chapter_number": slide.chapter_number,   # FIX-3
                            "section_number": slide.section_number,   # FIX-4
                            "chapter_title": slide.chapter_title,
                            "section_title": slide.section_title,
                            "course_id": course_id,
                        },
                    },
                    {
                        ...
                        "title": "Sélection du chapitre",
                        "summary": (
                            f"Chapitre {slide.chapter_number}: {slide.chapter_title}"
                        ),
                        "state": "presenting",
                        "status": "done",
                        "details": {
                            "chapter_index": slide.chapter_index,    # FIX-3: real index
                            "chapter_number": slide.chapter_number,
                            "section_index": slide.section_index,    # FIX-4: real index
                            "section_number": slide.section_number,
                            "slide_path": slide.slide_path,
                        },
                    },
                ...
                narration, audio_bytes = (
                    await self.presentation_service.explain_slide_focused(
                        slide,
                        ctx.student_profile,
                        ctx.language,
                        cancel_event=presentation_cancel_event,
                ...
                ctx.current_slide_narration = (
                    narration or slide.slide_content or "Présentation en cours…"
                )
                ...
                if audio_bytes:
                    await send_audio_stream(
                        audio_bytes, "audio/mpeg", clip=True, is_final=False
                    )
        ...
        # ------------------------------------------------------------------
        # FIX-2: Q&A uses dynamically detected language
        # ------------------------------------------------------------------
        async def handle_text_question_turn(
            question_text: str,
            language: str,
            ...
            # FIX-2: detect language from the student's actual text
            detected_lang = self._detect_language(question_text, fallback=language or ctx.language)
            effective_language = detected_lang
            log.info(
                f"[{session_id[:8]}] FIX-2: Q&A language detected: "
                f"{effective_language} (from: '{question_text[:30]}…')"
            )
            ...
            try:
                ...
                answer, _, metrics = await self.qa_service.process_text_question(
                    ...
                    ctx=ctx,
                    history=history,
                    language=effective_language,    # FIX-2
                    subject=subject,
                    on_step=emit_step_status,
                    on_audio_chunk=emit_audio_chunk,
                )
                ...
                await send({"type": "status_update", "state": "idle", "message": "Réponse envoyée"})
        ...
        async def handle_audio_question_turn(
            ...
            try:
                ...
                audio_np = audio_bytes_to_numpy(audio_bytes)
                text, stt_time, detected_language, lang_prob, _audio_duration = (
                ...
                # FIX-2: use the STT-detected language; fallback to session lang
                effective_language = (
                    detected_language
                    if detected_language and detected_language != "unknown"
                    else (question_language or ctx.language)
                )
                # Extra: cross-check with langdetect on the transcribed text
                if text and text.strip():
                    effective_language = self._detect_language(text, fallback=effective_language)
        ...
        async def cancel_tasks() -> None:
            ...
            presentation_cancel_event.set()
            for task in [
                audio_stream_task, presentation_task,
                text_question_task, next_slide_prefetch_task,
            ]:
                if task and not task.done():
                    task.cancel()
                    try:
        ...
        try:
            while not websocket_closed:
                try:
                    ...
                    # --------------------------------------------------
                    # FIX-3 / FIX-6: Session start + student profile
                    # --------------------------------------------------
                    if msg_type == "start_session":
                        token = message.get("token", "")
                        language = message.get("language", "fr")
                        ...
                        # FIX-6: return student profile in session_started
                        ...
                        if profile:
                            ...
                            try:
                                ...
                                profile_payload = {
                                    ...
                                    "language": getattr(profile, "language", language),
                                    "level": getattr(profile, "level", "lycée"),
                                    "name": getattr(profile, "name", "Étudiant"),
                                }
                        ...
                        await send({
                            ...
                            "session_id": session_id,
                            "language": language,
                            "student_profile": profile_payload,   # FIX-6
                        })
                        log.info(
                            f"✅ Session started: {session_id[:8]}, "
                            f"student: {student_id}, lang: {language}"
                        )

                    # --------------------------------------------------
                    # FIX-1 / FIX-3 / FIX-4 / FIX-5: start_presentation
                    # --------------------------------------------------
                    elif msg_type == "start_presentation":
                        if not ctx:
                            await send({"type": "error", "message": "Session not initialized"})
                        ...
                        # FIX-3: respect the chapter_index sent by frontend
                        chapter_idx = int(message.get("chapter_index", 0))
                        # FIX-4: respect the section_index sent by frontend
                        section_idx = int(message.get("section_index", 0))
                        ...
                        # FIX-1: We pass chapter/section; language sync happens
                        # inside handle_presentation_turn after loading slide metadata.
                        presentation_task = asyncio.create_task(
                            handle_presentation_turn(course_id, chapter_idx, section_idx)
                        )
                        ...
                        requested_language = str(message.get("language", "")).strip().lower()
                        question_language = (
                            None if requested_language in {"", "auto"} else requested_language
                        )
                        subject = message.get("subject", "") or message.get("topic", "")
                        ...
                        await cancel_tasks()
                        text_question_task = asyncio.create_task(
                            handle_text_question_turn(
                                question, language, subject, message.get("turn_id")
                            )
                        )
                        ...
                        if ctx:
                            await cancel_tasks()
                            ctx.state = DialogState.PAUSED.value
                            await send({"type": "paused", "checkpoint": {}, "resume_cursor": 0})
                    ...
                    await send({
                        "type": "status_update",
                        "state": "idle",
                        "message": "Session active (keepalive)",
                    })
                    continue
                except WebSocketDisconnect:
                    websocket_closed = True
            ...
            if ctx:
                self.analytics.record(
                    ...
                    {
                        "duration_minutes": (
                            datetime.utcnow() - ctx.created_at
                        ).total_seconds() / 60,
                        "interaction_count": ctx.interaction_count,
                    },
                    session_id=session_id,
...
def create_session_service(
    dialogue_mgr, rag, llm, voice, confusion_detector,
    transcript_search, analytics_engine, profile_mgr,
    media_service, course_analyzer,
) -> RealtimeSessionService:
    return RealtimeSessionService(
        dialogue_mgr=dialogue_mgr,
        rag=rag,
